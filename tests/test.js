...
// Force remote service to be mocked locally for tests
cds.env.requires.API_BUSINESS_PARTNER = { kind: 'odata-v2', model: 'srv/external/API_BUSINESS_PARTNER', credentials: { url: 'http://localhost' } }
...
describe('Test The GET Endpoints', () => {
  ...
  it('Test Expand Entity Endpoint', async () => {
    const { data } = await GET`/odata/v4/processor/Incidents?$select=title&$expand=customer`
